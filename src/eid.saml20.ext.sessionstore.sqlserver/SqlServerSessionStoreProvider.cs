...
namespace eid.saml20.ext.sessionstore.sqlserver
{
    /// <summary>
    ...
    public class SqlServerSessionStoreProvider : ISessionStoreProvider
    {
        ...
        {
            _connectionString = ConfigurationManager.ConnectionStrings["eidsaml:SqlServerSessionStoreProvider"]?.ConnectionString ?? throw new InvalidOperationException("The connectionstring \'eidsaml:SqlServerSessionStoreProvider\' must be set when using the SqlServerSessionStoreProvider");
            _schema = ConfigurationManager.AppSettings["eidsaml:SqlServerSessionStoreProvider:Schema"] ?? "dbo";

            int cleanupIntervalSeconds;
            if (int.TryParse(ConfigurationManager.AppSettings["eidsaml:SqlServerSessionStoreProvider:CleanupIntervalSeconds"], out cleanupIntervalSeconds))
            {
                _cleanupInterval = TimeSpan.FromSeconds(cleanupIntervalSeconds);
            }

            bool disableCleanup;
            if (!(bool.TryParse(ConfigurationManager.AppSettings["eidsaml:SqlServerSessionStoreProvider:DisableCleanup"], out disableCleanup) && disableCleanup))
            {
