...
namespace eid.saml20.Session
{
    internal static class SessionStore
    {
        static SessionStore()
        {
            var type = EidFederationConfig.GetConfig().SessionType;
            ...
            var sessionTimeoutMinutes = EidFederationConfig.GetConfig().SessionTimeout;
        ...
        {
            var name = EidFederationConfig.GetConfig().SessionCookieName;
            ...
            if (!DoesSessionExists())
            {
                if (CurrentSession == null)
                {
                    throw new Saml20Exception(
                        "The user doesn't have a session in context of a cookie ... which is required at this point in the pipeline. Plausible reason is that EIDSAML.Net is not running under https. The session cookie is marked with 'secure only'.");
                }
