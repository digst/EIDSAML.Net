...
namespace eid.saml20
{
    ...
    public class Saml20MetadataDocument
    {
        ...
        /// <param name="sign">if set to <c>true</c> the metadata document will be signed.</param>
        public Saml20MetadataDocument(EidSAML20FederationConfig config, Dictionary<KeyTypes, KeyInfo> keyinfos, bool sign)
            : this(sign)
        {
        ...
        /// </summary>        
        private void ConvertToMetadata(EidSAML20FederationConfig config, Dictionary<KeyTypes, KeyInfo> keyinfos)
        {
            ...
            if (config.RequestedAttributes.Attributes.Count > 0)
            {
                ...
                attConsumingService.ServiceName = new LocalizedName[] { new LocalizedName(config.ServiceProvider.Name, "da") };
                ...
                for (int i = 0; i < config.RequestedAttributes.Attributes.Count; i++)
                {
                    ...
                    attConsumingService.RequestedAttribute[i].FriendlyName = config.RequestedAttributes.Attributes[i].friendlyName;
                    ...
                    if(!string.IsNullOrWhiteSpace(config.RequestedAttributes.Attributes[i].ContextValue))
                        attConsumingService.RequestedAttribute[i].AttributeValue = new[] { config.RequestedAttributes.Attributes[i].ContextValue };
            ...
            keySigning.KeyInfo = Serialization.DeserializeFromXmlString<Schema.XmlDSig.KeyInfo>(keyinfos[KeyTypes.signing].GetXml().OuterXml);
            keyEncryption.KeyInfo = Serialization.DeserializeFromXmlString<Schema.XmlDSig.KeyInfo>(keyinfos[KeyTypes.encryption].GetXml().OuterXml);
        ...
        public string ToXml(Encoding enc)
        {
            ...
            if (Sign)
            {
                var metaDataShaHashingAlgorithm = EidFederationConfig.GetConfig().MetaDataShaHashingAlgorithm;
                ...
                var cert = EidFederationConfig.GetConfig().GetFirstValidCertificate();
