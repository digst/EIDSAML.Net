...
namespace eid.saml20.protocol
{
    ...
    public class Saml20SignonHandler : Saml20AbstractEndpointHandler
    {
        ...
        {
            _certificate = EidFederationConfig.GetConfig().GetFirstValidCertificate();

            // Read the proper redirect url from config
            try
            {
                RedirectUrl = EidSAML20FederationConfig.GetConfig().ServiceProvider.SignOnEndpoint.RedirectUrl;
                ErrorBehaviour = EidSAML20FederationConfig.GetConfig().ServiceProvider.SignOnEndpoint.ErrorBehaviour.ToString();
            }
            catch (Exception e)
            {
        ...
        private void SendRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(returnUrl) && (!EidFederationConfig.GetConfig().PreventOpenRedirectAttack || IsLocalUrl(returnUrl)))
        ...
        private void HandleResponse(HttpContext context)
        {
            ...
            {
                if (ex is Saml20LoaException)
                {
                    HandleError(context, ex.ToString(), (m) => new Saml20LoaException(m));
                }
                else
                {
                    HandleError(context, ex);
        ...
        private static Saml20EncryptedAssertion GetDecryptedAssertion(XmlElement elem)
        {
            ...
            var decryptionCertificate = EidFederationConfig.GetConfig().EncryptionCertificate.GetAllValidX509Certificates();
            ...
            try
            {
                return tryDecryptAssertion(decryptionCertificate);
            }
            catch (Exception)
            {                
                throw;
        ...
        {
            // If Eid LoA is missing, invalidate.
            var assertionEidLoa = GetEidLoa(assertion);
            if (assertionEidLoa == null)
            {
                AuditLogging.logEntry(Direction.IN, Operation.AUTHNREQUEST_POST, Resources.EidLoaMissing + " Assertion: " + assertionXml.OuterXml);
                HandleError(context, Resources.EidLoaMissing);
                return false;
            }

            return ValidateEidLoa(assertionEidLoa, context, assertionXml);
        }

        /// <summary>
        /// Validates if a LoA is equals to or higher than a minimum required LoA.
        /// If validation fails, response is modified to display an error page.
        /// </summary>
        /// <returns>True if valid, otherwise false (and modified response).</returns>
        private bool ValidateEidLoa(string loa, HttpContext context, XmlElement assertionXml)
        {
            var demandedLoa = SessionStore.CurrentSession[SessionConstants.ExpectedLoa]?.ToString();
            var minLoa = demandedLoa ?? EidSAML20FederationConfig.GetConfig().MinimumLoa;

            if (loa == minLoa) return true;

            switch (minLoa)
            {
                case "http://eidas.europa.eu/LoA/high" when loa != "http://eidas.europa.eu/LoA/high":
                case "http://eidas.europa.eu/LoA/substantial" when loa != "http://eidas.europa.eu/LoA/high" && loa != "http://eidas.europa.eu/LoA/substantial":
                    var msgTemplate = demandedLoa != null ?
                        Resources.LoaTooLowAccordingToDemand :
                        Resources.LoaTooLow;
                    HandleLoaValidationError(msgTemplate, loa, demandedLoa, context, assertionXml);
                    return false;
                default:
                    return true;
            }
        ...
        private void DoLogin(HttpContext context, Saml20Assertion assertion)
        {
            ...
            string assuranceLevel = GetEidLoa(assertion) ?? "(Unknown)";

            AuditLogging.logEntry(Direction.IN, Operation.LOGIN, string.Format("Subject: {0} NameIDFormat: {1}  Level of assurance: {2}  Session timeout in minutes: {3}", assertion.Subject.Value, assertion.Subject.Format, assuranceLevel, EidFederationConfig.GetConfig().SessionTimeout));
        ...
        /// </summary>
        /// <returns>Returns the LoA or null if it has not been defined.</returns>
        private string GetEidLoa(Saml20Assertion assertion)
        {
            var eidLoa = assertion.Assertion.GetAuthnStatements()
                .FirstOrDefault().AuthnContext.Items
                .FirstOrDefault()
                .ToString();
        ...
        private void TransferClient(IDPEndPoint idpEndpoint, Saml20AuthnRequest request, HttpContext context)
        {
            ...
            if (destination.Binding == SAMLBinding.POST)
            {
                ...
                var signingCertificate = EidFederationConfig.GetConfig().GetFirstValidCertificate();
