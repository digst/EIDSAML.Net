...
namespace eid.saml20.protocol
{
    ...
    public class Saml20MetadataHandler : AbstractEndpointHandler
    {
        ...
        {
            EidSAML20FederationConfig configuration = EidSAML20FederationConfig.GetConfig();

            var keyinfos = new Dictionary<KeyTypes, KeyInfo>();
            
            KeyInfo signingKeyinfo = new KeyInfo();
            KeyInfoX509Data signingKeyClause = new KeyInfoX509Data(EidFederationConfig.GetConfig().SigningCertificate.GetCertificate(), X509IncludeOption.EndCertOnly);
            signingKeyinfo.AddClause(signingKeyClause);
            keyinfos.Add(KeyTypes.signing, signingKeyinfo);

            KeyInfo encryptionKeyinfo = new KeyInfo();
            KeyInfoX509Data encryptionKeyClause = new KeyInfoX509Data(EidFederationConfig.GetConfig().EncryptionCertificate.GetCertificate(), X509IncludeOption.EndCertOnly);
            encryptionKeyinfo.AddClause(encryptionKeyClause);
            keyinfos.Add(KeyTypes.encryption, encryptionKeyinfo);
