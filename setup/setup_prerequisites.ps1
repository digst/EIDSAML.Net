...
$serviceprovidercertificate = Import-PfxCertificate '..\certificates\serviceprovider.pfx' -Password $certpassword2 -CertStoreLocation Cert:\LocalMachine\My
$serviceprovidercertificate = Import-PfxCertificate '..\certificates\serviceprovider.pfx' -Password $certpassword2 -CertStoreLocation Cert:\LocalMachine\TrustedPeople
...
#"http delete sslcert ipport=0.0.0.0:30001" | netsh
#"http delete sslcert ipport=0.0.0.0:30002" | netsh

#write-host "Registering demo idp ssl certificate $($demoIdpSslcertificate.Thumbprint) for SSL bindings for demo sites"
"http add sslcert ipport=0.0.0.0:30001 certhash=$($demoIdpSslcertificate.Thumbprint) appid={$([Guid]::NewGuid().ToString().ToUpper())}" | netsh
...
"http add sslcert ipport=0.0.0.0:30002 certhash=$($serviceProviderSslcertificate.Thumbprint) appid={$([Guid]::NewGuid().ToString().ToUpper())}" | netsh
...
add-HostEntry "127.0.0.1" "eidsaml-demoidp.dk"
add-HostEntry "127.0.0.1" "eidsaml-net.dk"
